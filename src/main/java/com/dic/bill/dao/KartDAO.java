...
public interface KartDAO extends JpaRepository<Kart, String> {
    ...
    @Query(value = "select distinct t.k_lsk_id from TESTPOLIS.KART t where t.reu=:reuId /*and t.PSCH not in (8,9)*/ order by t.k_lsk_id",
            nativeQuery = true)
    List<BigDecimal> findAllKlskIdByReuId(@Param("reuId") String reuId);

    @Query(value = "select distinct t.k_lsk_id from TESTPOLIS.KART t, TESTPOLIS.C_VVOD d where d.house_id=t.house_id " +
    ...
    @Query(value = "select distinct t.k_lsk_id from TESTPOLIS.KART t, TESTPOLIS.C_VVOD d, TESTPOLIS.NABOR n " +
    ...
    @Query(value = "select distinct t.k_lsk_id from TESTPOLIS.KART t /*where t.psch not in (8,9)*/ order by t.k_lsk_id", nativeQuery = true)
